...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Step 3: Query casetilla_salidas with scope + filters
    let query = supabase
      .from('casetilla_salidas')
      .select('id, org_id, reservation_id, chofer, matricula, dua, exit_at, created_by, created_at, fotos', { count: 'exact' })
      .eq('org_id', orgId)
      .in('reservation_id', visibleReservationIds);

    // from/to applies to exit_at (the actual exit timestamp)
    if (from) query = query.gte('exit_at', from);
    if (to) query = query.lte('exit_at', to);
    if (matriculaFilter) query = query.ilike('matricula', `%${matriculaFilter}%`);
    if (duaFilter) query = query.ilike('dua', `%${duaFilter}%`);

    query = query.order('exit_at', { ascending: false }).range(offset, offset + pageSize - 1);

    const { data: salidas, error: queryError, count } = await query;
    if (queryError) {
      return new Response(JSON.stringify({ error: 'Error fetching casetilla_salidas', details: queryError.message }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const shaped = (salidas || []).map((sal: any) => {
      const res = sal.reservation_id ? reservationMap.get(sal.reservation_id) : null;
      ...
      return {
        id: sal.id,
        org_id: sal.org_id,
        reservation_id: sal.reservation_id,
        chofer: sal.chofer,
