...
import type { ActiveWarehouseInfo } from './CollaboratorModal';
import { ManpowerControlModal } from './ManpowerControlModal';
...
export default function CollaboratorsTab() {
  ...
  // Derivar país y almacén del switch (almacén activo)
  const activeWarehouseInfo = useMemo<ActiveWarehouseInfo | null>(() => {
    ...
    return {
      id: wh.id,
      name: wh.name,
      country_id: wh.country_id ?? '',
      country_name: wh.country_id ? countryNameById.get(wh.country_id) ?? '' : ''
    };
  }, [activeWarehouseId, warehouses, countryNameById]);
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-end gap-3 mb-6">
        ...
        >
          <i className="ri-bar-chart-box-line text-lg w-5 h-5 flex items-center justify-center"></i>
          <span>Control</span>
        </button>
        {canManage && (
          <button
        ...
        <div className="mb-4 bg-teal-50 border border-teal-200 rounded-xl px-5 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-teal-900">
              <span className="font-semibold">Tenés un borrador de colaborador sin finalizar</span>
              <span className="text-teal-700 ml-1">({resumeDraftAge})</span>
            </p>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              onClick={() => { setShowResumeBanner(false); openCreateModal(); }}
            ...
            <button
              onClick={() => { localStorage.removeItem(`draft_collaborator_${orgId}_new`); setShowResumeBanner(false); }}
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        ...
        {!viewAll && !selectedCountry && !selectedWarehouse && (
          <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start space-x-2">
            <i className="ri-information-line text-amber-600 text-lg mt-0.5 w-5 h-5 flex items-center justify-center"></i>
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Almacenes</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipo de Trabajo</th>
                  ...
                  {paginatedCollaborators.map((collaborator) => (
                    <tr key={collaborator.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{collaborator.full_name}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-600">{collaborator.ficha || '-'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-600">{collaborator.cedula || '-'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-600">{countryNameById.get(collaborator.country_id) || '-'}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1">
                          {collaborator.warehouses && collaborator.warehouses.length > 0 ? (
                            collaborator.warehouses.map((warehouse) => (
                              <span key={warehouse.id} className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-teal-100 text-teal-800">
                                {warehouse.name}
                              </span>
                            ))
                          ) : (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-600">{workTypeNameById.get(collaborator.work_type_id) || '-'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${collaborator.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
            ...
            {totalPages > 1 && (
              <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
                <div className="text-sm text-gray-600">
                  Mostrando {(currentPage - 1) * itemsPerPage + 1} a {Math.min(currentPage * itemsPerPage, filteredCollaborators.length)} de {filteredCollaborators.length} colaboradores
                </div>
